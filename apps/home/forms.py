...
TERRITORIAL_MANAGERS = list(set(Location.objects.values_list("territorial_manager", flat=True)))
...
TUTORS_PROGRAMMING = list(Location.objects.exclude(tutor__isnull=True).values_list("tutor", flat=True))
TUTORS_ENGLISH = list(Location.objects.exclude(tutor_english__isnull=True).values_list("tutor_english", flat=True))
...
class AddCMForm(forms.Form):
    client_manager = forms.ChoiceField(label='Клієнтський менеджер', choices=CLIENT_MANAGERS, required=True)
...
class ReasonForRevert(forms.Form):
    reason = forms.CharField(label='Причина повернення незбіжності', max_length=1024, required=True)
